...
#+BEGIN_SRC emacs-lisp
  (bind-key (kbd "C-c t") #'jtg:jump-to-or-start-term)
#+END_SRC
...
View mode doesn't provide for wrapping long lines, which is annoying. I'd like
to ensure that it does so, so I provide a hook for this.

#+BEGIN_SRC emacs-lisp
  (defun jtg:wrap-lines-hook ()
    "Wraps long lines by setting truncate-lines to nil and enabling visual line
  mode (if available)"
    (setq truncate-lines nil
          word-wrap t)
    (visual-line-mode))

  (add-hook 'view-mode-hook #'jtg:wrap-lines-hook)
#+END_SRC
...
Add objc-mode support for objc++ files since there's no actual mode for it, and
objc-mode comes close.
  ...
  (if (boundp 'inferior-lisp-program)
      (progn
        (cond ((or (eq system-type 'darwin)
              ...
              (t (setq-default inferior-lisp-program "/usr/bin/sbcl")))
